...
class MapGl extends Component {
    ...
    fly = (map, origin, destination, pulsingDotId, pulsingDot, sourceId) => {
        ...
        /* fly-start */
        /* 吧起点转为前一次终点，终点随机3~5 */
        /* let origin = destinationSource;
        let destination = [(destinationSource[0] + (Math.random(1) + 3)), (destinationSource[1] + (Math.random(1) + 3))]
console.log(origin, destination) */
        ...
        //画一条弧之间的“起源”&“目的地”的两个点
        var segment = [origin[0], origin[1]];
        for (var i = 0; i < 231; i++) {
            /* var segment = turf.along(route.features[0], i, { units: 'kilometers' });
            arc.push(segment.geometry.coordinates); */
            segment = [(segment[0] + Math.random(0.4) + 0.1), (segment[1] + Math.random(0.4) + 0.1)]
            arc.push(segment)
        }
